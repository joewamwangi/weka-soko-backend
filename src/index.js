...
io.on("connection", (socket) => {
  ...
  socket.on("join_listing", async (listingId) => {
    try {
      ...
      // Check if user has any message history on this listing
      ...
      socket.canChat = isSeller || isAdmin || msgCheck.length > 0 || true; // allow all to start chat
      socket.join(`listing:${listingId}`);
      socket.emit("joined", { listingId, canChat: socket.canChat, isSeller });
    } catch {
