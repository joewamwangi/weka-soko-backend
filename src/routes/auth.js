...
router.delete("/account", requireAuth, async (req, res, next) => {
  try {
    ...
    await withTransaction(async (client) => {
      // 1. Null-out FK references that have no cascade (prevent constraint violations)
      ...
      // 2. Delete this user's chat messages
      await client.query(`DELETE FROM chat_messages WHERE sender_id = $1 OR receiver_id = $1`, [uid]);
      await client.query(`DELETE FROM messages WHERE sender_id = $1 OR recipient_id = $1`, [uid]).catch(()=>{});
      await client.query(`DELETE FROM inbox_messages WHERE recipient_id = $1`, [uid]).catch(()=>{});

      // 3. Delete their listings (cascade handles listing_photos, chat_messages on listing, etc.)
      await client.query(`DELETE FROM listings WHERE seller_id = $1`, [uid]);

      // 4. Delete their escrows, disputes, violations, notifications
      ...
      await client.query(`DELETE FROM vouchers WHERE created_by = $1`, [uid]).catch(()=>{});

      // 5. Hard delete the user — all data gone, email freed up but we scramble it first
      // to prevent re-registration with same email after deletion request
      const scrambledEmail = `deleted_${uid}_${Date.now()}@wekasoko.deleted`;
      await client.query(`UPDATE users SET email = $1 WHERE id = $2`, [scrambledEmail, uid]);
...
router.get("/google/callback", async (req, res) => {
  ...
  try {
    ...
    const user = rows[0];
    if (rows[0].account_status === "deleted") throw new Error("This account has been deleted.");
