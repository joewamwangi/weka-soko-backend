...
// ── MALLS (Stores) Admin Routes ───────────────────────────────────────────────

// GET /api/admin/stores — all stores
router.get("/stores", async (req, res, next) => {
  try {
    const { status, q, limit = 40, offset = 0 } = req.query;
    const conditions = [];
    const params = [];
    if (status) { params.push(status); conditions.push(`s.status=$${params.length}`); }
    if (q) { params.push(`%${q}%`); conditions.push(`(s.name ILIKE $${params.length} OR s.slug ILIKE $${params.length})`); }
    const where = conditions.length ? "WHERE " + conditions.join(" AND ") : "";
...
// PATCH /api/admin/stores/:id/status — approve / suspend / reject
router.patch("/stores/:id/status", async (req, res, next) => {
  try {
    const { status, rejection_reason, admin_note } = req.body;
    const VALID = ["pending","active","suspended","rejected"];
    if (!VALID.includes(status)) return res.status(400).json({ error: "Invalid status" });
    const { rows } = await query(
      `UPDATE stores SET status=$1, rejection_reason=$2, admin_note=$3, updated_at=NOW()
       WHERE id=$4 RETURNING *`,
      [status, rejection_reason||null, admin_note||null, req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: "Store not found" });
    const store = rows[0];
    // Notify owner
    const { rows: ownerRows } = await query(`SELECT name, email FROM users WHERE id=$1`, [store.owner_id]);
    if (ownerRows.length) {
      const { sendEmail } = require("../services/email.service");
      const msgs = {
        active: { subj:"Your Weka Soko Mall is now live!", body:`Hi ${ownerRows[0].name},\n\nGreat news! Your Weka Soko Mall "${store.name}" has been approved and is now live.\n\nYour store URL: ${process.env.FRONTEND_URL}/store/${store.slug}\n\nStart adding products and share your store link with customers.\n\n— Weka Soko` },
        suspended: { subj:"Your Weka Soko Mall has been suspended", body:`Hi ${ownerRows[0].name},\n\nYour Mall "${store.name}" has been temporarily suspended. Reason: ${admin_note||"Policy violation"}.\n\nContact support@wekasoko.co.ke to appeal.\n\n— Weka Soko` },
        rejected: { subj:"Weka Soko Mall application", body:`Hi ${ownerRows[0].name},\n\nUnfortunately your Mall application for "${store.name}" was not approved. Reason: ${rejection_reason||"Does not meet requirements"}.\n\nYou may apply again after addressing the issue.\n\n— Weka Soko` },
      };
      if (msgs[status]) sendEmail(ownerRows[0].email, ownerRows[0].name, msgs[status].subj, msgs[status].body).catch(()=>{});
    }
    res.json(store);
  } catch (err) { next(err); }
});

// PATCH /api/admin/stores/:id/verify — toggle verified badge
router.patch("/stores/:id/verify", async (req, res, next) => {
  try {
    const { rows: cur } = await query(`SELECT is_verified FROM stores WHERE id=$1`, [req.params.id]);
    if (!cur.length) return res.status(404).json({ error: "Store not found" });
    const nowVerified = !cur[0].is_verified;
    const { rows } = await query(
      `UPDATE stores SET is_verified=$1, verified_at=$2, verified_by=$3, updated_at=NOW()
       WHERE id=$4 RETURNING *`,
      [nowVerified, nowVerified ? new Date() : null, nowVerified ? req.user.id : null, req.params.id]
    );
    res.json(rows[0]);
  } catch (err) { next(err); }
});
...
router.delete("/stores/:id", async (req, res, next) => {
  try {
    // Unlink listings from the store first
    await query(`UPDATE listings SET store_id=NULL WHERE store_id=$1`, [req.params.id]);
    await query(`UPDATE users SET store_id=NULL WHERE store_id=$1`, [req.params.id]);
